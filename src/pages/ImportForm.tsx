...
enum ImportStatus {default, success, collisions, error, networkFailure}
...
export default function ImportForm() {
  ...
  let [collisionsData, setCollisionsData] = useState<{schedulesWithConflicts: []}>({schedulesWithConflicts: []});
  let [response, setResponse] = useState<{status: number, statusText:string}>({status: -1, statusText: "" });
  let [error, setError] = useState<{message :string}>({message: "" });

  const showMessage = () => {
    switch (importStatus){
      case ImportStatus.default: return <></>;
      case ImportStatus.success: return ImportSuccess(successData)
      case ImportStatus.collisions: return ImportCollisions(collisionsData)
      case ImportStatus.error: return ImportError(response)
      case ImportStatus.networkFailure: return ImportNetworkFailure(error)
    }
  }
