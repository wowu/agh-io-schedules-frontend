...
export default function ImportSuccess(props: { uploaded: [] }) {
  ...
  const getMessage = (schedule: any) => {
    return (
      <span>{schedule.name} : {schedule.eventCount} wydarzenia od {new Date(schedule.firstEventDate).toLocaleDateString()} do {new Date(schedule.lastEventDate).toLocaleDateString()}
        </span>
    );
  };
